...
classdef DGPNode < handle
    %This is an abstract class for a GP, which is used as a node in the
    %contruction of DGP
    ...
    properties
        ...
        dgp          % Object. The DGP obejct where this node belongs to
    ...
    methods
        function obj = DGPNode(father, role, g, name, gp_ker, hyper_para)
            ...
            if strcmp(g, 'exp')
                ...
                obj.dg = @(l) exp(l);
                obj.g = g;
            elseif strcmp(g, 'softplus')
                g = @(l) log(exp(l)+1);
                obj.dg = @(l)exp(l) ./ (exp(l)+1);
                obj.g = g;
        ...
        function [hyper_para, neg_log_likeli] = hyper_opt_ns(obj, x, y, R, options)
            ...
            if nargin < 5
                options = optimoptions('fmincon','Algorithm','Interior-Point', ...
                                    ...
                                    'iter-detailed', 'MaxIterations', 200, ...
        ...
        function [hyper_para, neg_log_likeli] = hyper_opt_ns_gd(obj, x, y, R, options, method)
            % Optimizing hyperparameters on mariginal likelihood on a
            % non-stationary GP.
            %
            % Arguments:
            ...
            if strcmp(method, 'RMSPROP')
                [hyper_para, neg_log_likeli] = opts.rmsprop(obj.hyper_para(:), opt_func, options);
            elseif strcmp(method, 'ADAM')
                [hyper_para, neg_log_likeli] = opts.adam(obj.hyper_para(:), opt_func, options);
            else
...
%             [hyper_para, neg_log_likeli] = fminunc(opt_func, obj.hyper_para(:), options_unc);
