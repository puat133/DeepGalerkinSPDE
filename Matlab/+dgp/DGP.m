...
%             end
        ...
        function [m, cov] = predict(obj, node, query, int_method)
            ...
            % Simply approximate
            % p(f|y_1:N) \approx p(f|ℓ_query, σ_query, y_1:N)
            % as a non-stationary GP, where ℓ_query and σ_query are
            ...
            % Plan A: p(f|y_1:N) \approx p(f|ℓ_query, σ_query, y_1:N)
            % Plan B: p(f|y_1:N) \approx p(f|ℓ_query, σ_query, f_MAP)
